<?php

return [
    'programs' => "Program",
    'science' => 'Sains',

    'science_title' => 'Sains',
    'science_description' => 'Bergabunglah dalam petualangan yang mendebarkan  untuk menjelajahi keajaiban alam, membangkitkan rasa ingin tahu dan pemikiran kritis melalui eksperimen dan penemuan yang menarik, memberdayakan anak untuk menjadi penemu dan inovator.',

    'science_image' => '',
    'science_why_badge' => 'MENGAPA',
    'science_why_learn' => 'Mengapa perlu belajar Sains?',
    'science_why_description' => 'Studi terbaru membuktikan bahwa program sains yang terintegrasi dengan pendidikan alam dan lingkungan bermanfaat untuk meningkatkan kesadaran anak, kemampuan dalam memecahkan masalah dan fleksibilitas kognitif. Pemaparan awal terhadap sains terbukti mampu membantu mengembangkan penalaran spasial dan memicu semangat untuk pembelajaran berbasis inkuiri, sehingga berkontribusi signifikan terhadap pertumbuhan intelektual anak. Melalui praktek langsung dan eksplorasi, anak tidak hanya memperoleh pengetahuan dasar tentang dunia tetapi juga mengembangkan keterampilan berpikir kritis yang diperlukan dalam kehidupan sehari-hari.
    Bergabunglah dengan kami di EduALL Junior untuk mengembangkan rasa ingin tahu anak Anda dan membentuk landasan pengetahuan dan literasi ilmiah seumur hidup.',

    'what_your_kids_learn_title' => 'Apa yang akan dipelajari anak Anda',
    'what_your_kids_learn_content' => [
        [
            'title' => 'Melibatkan Eksplorasi dan Penemuan dalam Kehidupan Sehari-hari dan Lingkungan',
            'description' => 'Program Sains EduALL Junior menawarkan kegiatan menarik yang dirancang khusus untuk anak-anak usia 5-12 tahun. Melalui berbagai eksperimen praktis seperti mengamati fenomena alam hingga menciptakan reaksi kimia sederhana, murid akan menjelajahi berbagai konsep ilmiah. Selain itu, diskusi interaktif tentang topik seperti ekosistem dan ilmu fisika akan memupuk rasa ingin tahu dan pemahaman mendalam tentang prinsip-prinsip ilmiah yang mendasar.',
        ],
        [
            'title' => 'Kurikulum Progresif dengan Aplikasi Dunia Nyata',
            'description' => 'Dikemas dalam level yang sesuai dengan usia, setiap sesi membangun pengetahuan anak-anak secara progresif. Dengan fokus pada observasi dan aplikasi dunia nyata, program kami mengintegrasikan pembelajaran di kelas dengan praktik langsung yang relevan, seperti eksperimen dengan sumber energi terbarukan dan eksplorasi ilmu lingkungan. Melalui pembelajaran berbasis inkuiri dan kegiatan interaktif, anak-anak tidak hanya memahami konsep ilmiah tetapi juga mengembangkan keterampilan berpikir kritis, pemecahan masalah, dan kolaborasi yang esensial untuk sukses di dunia saat ini.',
        ]
    ],

    'learn_more_title' => 'Pelajari lebih lanjut tentang',
    'learn_more_see_more' => 'Lihat lebih banyak',
    'learn_more_highlight' => 'Sains',
    'learn_more_content' => [
        [
            'image' => '',
            'title' => 'Trials & Errors for Kids',
            'description' => 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse varius enim in eros. Placerat dignissim duis ultrices gravida pharetra rhoncus adipiscing.',
            'read_more_badge' => 'Read More',
        ],
        [
            'image' => '',
            'title' => 'How to Teach AI to Kids',
            'description' => 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse varius enim in eros. Placerat dignissim duis ultrices gravida pharetra rhoncus adipiscing.',
            'read_more_badge' => 'Read More',
        ],
        [
            'image' => '',
            'title' => 'Science in Real-World Application',
            'description' => 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse varius enim in eros. Placerat dignissim duis ultrices gravida pharetra rhoncus adipiscing.',
            'read_more_badge' => 'Read More',
        ],
    ],

    'testimonials_badge' => 'TESTIMONIALS',
    'testimonials_title' => 'Apa yang orang tua katakan <span class="text-blue">tentang kami</span>',
];
