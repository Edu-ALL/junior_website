    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('testi_thumbnail')) {
                ...
                $destinationPath = 'project/junior-website/testimonial/' . date('Y') . '/' . date('m') . '/';
                $time = $testi_en->group;
    ...
    public function update($group, Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('testi_thumbnail')) {
                $file = $request->file('testi_thumbnail');
                $file_format = $request->file('testi_thumbnail')->getClientOriginalExtension();
                $destinationPath = 'project/junior-website/testimonial/' . $testi_en->created_at->format('Y') . '/' . $testi_en->created_at->format('m') . '/';
                $time = $testi_en->group;
                $fileName = 'Testimonial-thumbnail-' . $time . '.' . $file_format;
                Storage::disk('s3')->put($destinationPath . $fileName, file_get_contents($file));
                $testi_en->testi_thumbnail = $fileName;
                ...
                if ($testi_en->testi_thumbnail == $testi_id->testi_thumbnail) {
                    ...
                    $file_path = 'project/junior-website/testimonial/' . $testi_en->created_at->format('Y') . '/' . $testi_en->created_at->format('m') . '/' . $old_image_path;
    ...
    public function delete($group)
    {
        ...
        try {
            ...
            if ($old_image_path_en = $testi[0]->testi_thumbnail) {
                $file_path_en = 'project/junior-website/testimonial/' . $testi[0]->created_at->format('Y') . '/' . $testi[0]->created_at->format('m') . '/' . $old_image_path_en;
                if (Storage::disk('s3')->exists($file_path_en)) {
                    Storage::disk('s3')->delete($file_path_en);
                }
            }
            if ($old_image_path_id = $testi[1]->testi_thumbnail) {
                $file_path_id = 'project/junior-website/testimonial/' . $testi[1]->created_at->format('Y') . '/' . $testi[1]->created_at->format('m') . '/' . $old_image_path_id;
